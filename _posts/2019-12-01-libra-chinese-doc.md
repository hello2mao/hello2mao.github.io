---
layout: post
title: "Libra中文技术解读整理"
subtitle: "Libra Chinese"
date: 2019-12-01 10:51:11
author: "hello2mao"
hidden: true
tags:
  - blockchain
---

<!-- TOC -->

- [1. Overview](#1-overview)
- [2. Admission Control](#2-admission-control)
- [3. Bytecode Verifier](#3-bytecode-verifier)
- [4. Consensus](#4-consensus)
- [5. Crypto](#5-crypto)
- [6. Execution](#6-execution)
- [7. Mempool](#7-mempool)
- [8. Move IR Compiler](#8-move-ir-compiler)
- [9. Move Language](#9-move-language)
- [10. Network](#10-network)
- [11. Storage](#11-storage)
- [12. Virtual Machine](#12-virtual-machine)
- [13. Other](#13-other)

<!-- /TOC -->

> Libra中文资料比较少，技术解读就更加少了，本文按照关键模块，简单整理了下

# 1. Overview
- [Libra白皮书](https://libra.org/zh-CN/wp-content/uploads/sites/17/2019/06/LibraWhitePaper_zh_CN-1.pdf)
- [Libra技术原理浅析（一）：基本设计](https://www.raydbg.com/2019/Introduction-to-Libra-1/)
- [Libra 白皮书技术解读](https://learnblockchain.cn/2019/08/07/libra-tech/)

# 2. Admission Control
- [Libra 源码分析：Libra 的准入控制 (AC) 模块](https://learnblockchain.cn/2019/07/02/AC-module/)

# 3. Bytecode Verifier

# 4. Consensus
- [【Libra 技术解读】详解LibraBFT共识机制](https://mp.weixin.qq.com/s/oyu9ndRPvzCFWF7QcH_tvA)
- [【Libra 技术解读】详解Libra共识源码](https://mp.weixin.qq.com/s/mYchizsguCyfbQnltRrjbQ)
- [LibraBFT算法简述](https://zhuanlan.zhihu.com/p/69941436)
- [突破区块链不可能三角（四）——区块链中的BFT及HotStuff BFT（Libra BFT）分析](https://zhuanlan.zhihu.com/p/78772451)

# 5. Crypto
- [LIBRA 中的可验证随机数 VRF](https://learnblockchain.cn/2019/07/14/libra-VRF/)

# 6. Execution

# 7. Mempool
- [Libra 源码分析：内存池 mempool 模块解读 - 1](https://learnblockchain.cn/2019/07/03/libra-mempool-module1/)
- [Libra 源码分析：内存池 mempool 模块解读 - 2](https://learnblockchain.cn/2019/07/04/libra-mempool-module2/)
- [Libra 源码分析：内存池 mempool 模块解读 - 3](https://learnblockchain.cn/2019/07/05/libra-mempool-module3/)

# 8. Move IR Compiler

# 9. Move Language
- [Libra技术原理浅析（二）：Move语言](https://www.raydbg.com/2019/Introduciton-to-Libra-2/)
- [解读 Libra Move：一种可编程资源语言](https://learnblockchain.cn/2019/06/28/deep-move/)
- [伯克利女程序员硬核详解 Libra 编程语言 Move 的主要特征及基本语法](https://www.chainnews.com/articles/775489969485.htm)
- [move语言简介](https://mp.weixin.qq.com/s/cXBECPk_zIs22YlQyyqeug)
- [Libra 技术解读！详解move语法、解释器和介绍器](https://mp.weixin.qq.com/s/9MyNnhmaXa3tIN2zaFgKMg)


# 10. Network

# 11. Storage
- [Libra 源码分析：Libra 中数据存储的 Schema](https://learnblockchain.cn/2019/06/30/Schema-for-data-storage-in-Libra/)
- [Libra 源码分析：打通 Libra CLI 客户端与 libradb 模块](https://learnblockchain.cn/2019/07/01/Proficient-client-and-libradb-modules/)

# 12. Virtual Machine

# 13. Other
- [通过 Libra 学习 Protobuf](https://learnblockchain.cn/2019/06/29/Learn-Protobuf-in-Libra/)
- [Libra第一笔转账——硬核方式](https://mp.weixin.qq.com/s/EjXPKupRX-qkIt_f8yBJbA)
- [Accumulator Proof解析](https://mp.weixin.qq.com/s/BIl8ucIEi1odFskmPL8oBg)
- [如何黑盒破解 Libra 的序列化算法](https://mp.weixin.qq.com/s/ZEjYqlBUNPBbPmhKK-eKxg)